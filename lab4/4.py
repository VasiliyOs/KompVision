...
def hysteresis_thresholding(nms, low_thresh, high_thresh):
    ...
    for i in range(h):
        for j in range(w):
            if edges[i, j] == strong and not visited[i, j]:
                flood_fill(i, j)
...
def main():
    ...
    low_thresh = max_grad / 11
    high_thresh = max_grad / 4

    final_edges = hysteresis_thresholding(nms, low_thresh, high_thresh)

    print(f"max_grad: {max_grad}")
    print(f"low_thresh: {low_thresh:.2f}")
    print(f"high_thresh: {high_thresh:.2f}")


    cv2.imshow("Original", cv2.resize(blurred, (600, 700)))
    cv2.imshow("NMS", cv2.resize(nms, (600, 700)))
    cv2.imshow("Final Edges (Canny)", cv2.resize(final_edges, (800, 700)))
